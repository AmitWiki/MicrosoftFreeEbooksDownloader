...
$FileUrls = @('http://ligman.me/29ngkYn',
...
$DestinationFolder = "$HOME\downloads\Microsoft_Ebooks"
Write-Host "Checking if destination folder($DestinationFolder) already exists"
if(Test-Path -Path $DestinationFolder) {
    Write-Host "Destination folder ($DestinationFolder) already exists, proceeding with download."
} else {
    Write-Host "Destination folder ($DestinationFolder) doesn't exist, creating it."
    New-Item -ItemType Directory -Force -Path $DestinationFolder
}
Write-Host "Downloading books..."
$FileUrls | Foreach {
        Save-FileUrlFile -FileUrl $_ -DestinationFolder $DestinationFolder
}
